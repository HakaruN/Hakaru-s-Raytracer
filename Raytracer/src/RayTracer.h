...
public:
	...
	static void CheckIntersect(Ray ray, float t, Sphere lighting, Sphere targetSphere, float* frameBuffer, float* depthBuffer, int bufferWidth, int x, int y);
	static Sphere loadSphere(Vector pos, float size, Colour colour);
	static void Render(int bufferWidth, int widthStart, int widthEnd, int heighStart, int heightEnd, float* frameBuffer, float* depthBuffer, float &t, Sphere &lighting, std::vector<Sphere> &spheres);
